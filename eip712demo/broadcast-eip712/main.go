...
func buildEIP712Tx(
	...
	// Parse EVM chain ID from the signed transaction
	var evmChainID uint64
	if signedTx.TypedData.Domain.ChainId != nil {
		// ChainId is *math.HexOrDecimal256, which is just a *big.Int wrapper
		evmChainID = (*big.Int)(signedTx.TypedData.Domain.ChainId).Uint64()
	} else {
		return nil, fmt.Errorf("EVM chain ID not found in typed data domain")
	}

	// Ensure signature has correct recovery ID format (0 or 1, not 27/28)
	adjustedSig := make([]byte, len(signature))
	copy(adjustedSig, signature)
	if adjustedSig[crypto.RecoveryIDOffset] == 27 || adjustedSig[crypto.RecoveryIDOffset] == 28 {
		adjustedSig[crypto.RecoveryIDOffset] -= 27
	}

	// Add EIP-712 extension with the actual signature
	option, err := codectypes.NewAnyWithValue(&ethermint.ExtensionOptionsWeb3Tx{
		TypedDataChainID: evmChainID,
		FeePayer:         signedTx.Bech32Address,
		FeePayerSig:      adjustedSig,
	})
	if err != nil {
		return nil, fmt.Errorf("failed to create extension option: %w", err)
	}
	builder.SetExtensionOptions(option)

	// Set EMPTY signature with SIGN_MODE_LEGACY_AMINO_JSON
	// The actual signature is in ExtensionOptionsWeb3Tx.FeePayerSig
	sigData := &signingtypes.SingleSignatureData{
		SignMode:  signingtypes.SignMode_SIGN_MODE_LEGACY_AMINO_JSON,
		Signature: []byte{}, // Must be empty for EIP-712
	}

#!/bin/bash
# Compare mint module parameters between network and proposal file
#
# This script queries a network's current mint module parameters and compares
# them with a proposed mint parameter update.
#
...
# example:
# ./scripts/compare_mint_params.sh proposals/example-mint-proposal.json
...
if [ -z "$1" ]; then
    ...
    echo "  $0 proposals/example-mint-proposal.json"
    echo ""
    echo "  # Compare against a different network:"
    echo "  $0 proposals/example-mint-proposal.json https://rpc.testnet.shardeum.org:443 shardeum_8119-1"
    echo ""
...
echo -e "${BLUE}╔════════════════════════════════════════════════════════════════╗${NC}"
echo -e "${BLUE}║           Mint Module Parameters Comparison                    ║${NC}"
echo -e "${BLUE}╚════════════════════════════════════════════════════════════════╝${NC}"
echo ""
...
echo -e "${YELLOW}Query Command Used:${NC} $BINARY query mint params --node $NODE --chain-id $CHAIN_ID"
echo ""
CURRENT_PARAMS=$($BINARY query mint params --node "$NODE" --chain-id "$CHAIN_ID" --output json 2>/dev/null)
...
# Compare all mint module parameters
compare_param "Mint Denom" '.mint_denom'
compare_param "Inflation Rate Change" '.inflation_rate_change'
compare_param "Inflation Max" '.inflation_max'
compare_param "Inflation Min" '.inflation_min'
compare_param "Goal Bonded" '.goal_bonded'
compare_param "Blocks Per Year" '.blocks_per_year'

...
- [ ] Bump **`shardeum/shardeum-cosmos` to v0.4.0** and align **Cosmos SDK / IBC / CometBFT** constraints.
...
Bump the `shardeum/shardeum-cosmos` dependency in `go.mod` and tidy modules:
*(go.mod)*

```diff
- github.com/shardeum/shardeum-evm v0.3.1
+ github.com/shardeum/shardeum-evm v0.4.0
```
...
- [ ] `go.mod` has **no `replace`** lines for `github.com/shardeum/shardeum-evm`.

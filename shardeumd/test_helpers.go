...
func setup(withGenesis bool, invCheckPeriod uint, chainID string, evmChainID uint64) (*ShardeumApp, GenesisState) {
	db := dbm.NewMemDB()
	...
	app := NewShardeumApp(log.NewNopLogger(), db, nil, true, appOptions, evmChainID, testconfig.EvmAppOptions, baseapp.SetChainID(chainID))
	if withGenesis {
...
// Setup initializes a new ShardeumApp. A Nop logger is set in ShardeumApp.
func Setup(t *testing.T, chainID string, evmChainID uint64) *ShardeumApp {
	t.Helper()
...
// SetupWithGenesisValSet initializes a new ShardeumApp with a validator set and genesis accounts
// that also act as delegators. For simplicity, each validator is bonded with a delegation
// of one consensus engine unit in the default token of the simapp from first genesis
// account. A Nop logger is set in ShardeumApp.
func SetupWithGenesisValSet(t *testing.T, chainID string, evmChainID uint64, valSet *cmttypes.ValidatorSet, genAccs []authtypes.GenesisAccount, balances ...banktypes.Balance) *ShardeumApp {
	t.Helper()

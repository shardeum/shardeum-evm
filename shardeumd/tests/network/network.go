...
func DefaultConfig() Config {
	...
	cfg := Config{
		...
		ChainID:           chainID,
		NumValidators:     4,
		BondDenom:         testconstants.ShardeumAttoDenom,
		MinGasPrices:      fmt.Sprintf("0.000006%s", testconstants.ShardeumAttoDenom),
...
// NewAppConstructor returns a new Shardeum AppConstructor
...
func New(l Logger, baseDir string, cfg Config) (*Network, error) {
	...
	for i := 0; i < cfg.NumValidators; i++ {
		...
		nodeDir := filepath.Join(network.BaseDir, nodeDirName, "shardeumd")
		clientDir := filepath.Join(network.BaseDir, nodeDirName, "shardeumcli")

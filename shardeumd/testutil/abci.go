...
func DeliverTx(
	...
	txConfig := shardeumApp.GetTxConfig()
	...
	return BroadcastTxBytes(shardeumApp, txConfig.TxEncoder(), tx)
}
...
func DeliverEthTx(
	...
	res, err := BroadcastTxBytes(shardeumApp, txConfig.TxEncoder(), tx)
	if err != nil {
		return res, err
	}

	codec := shardeumApp.AppCodec()
...
func CheckTx(
	...
	return checkTxBytes(shardeumApp, txConfig.TxEncoder(), tx)
}
